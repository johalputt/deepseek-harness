...
The `web_fetch` implementation is an anonymous public HTTP(S) fetch provider, `http`. It fetches bytes from a concrete URL, applies the basic transport hygiene below (http/https-only, credential rejection, byte/time caps, cross-origin redirect blocking), decodes textual content, and returns only the minimal model-useful result: final URL, status code, body, and truncation. It carries no browser cookies, editor credentials, git credentials, internal auth tokens, or implicit access to private services. (Private-network destination blocking is enforced by `blockPrivateNetworks` — see [the SSRF Agent Note](2026-08-25-web-fetch-ssrf-blocking.md).)
...
SSRF / private-network protection has landed in the `http` provider: `blockPrivateNetworks` (default enabled) refuses destinations whose host resolves to any non-public address, with a pre-flight resolution plus a connect-time re-validation inside the dispatcher and per-hop re-validation across redirects — see [the SSRF Agent Note](2026-08-25-web-fetch-ssrf-blocking.md) for the mechanism and its remaining limits.
...
**Fetch is a network boundary, not just a read-only tool.** `web_fetch` can reach sensitive network targets or exfiltrate data through URLs. The transport hygiene ships (http/https-only, credential rejection, byte/time caps, cross-origin redirect blocking) together with private-network destination blocking ([SSRF Agent Note](2026-08-25-web-fetch-ssrf-blocking.md)); deployments that fetch loopback or LAN services must opt out via the provider's `blockPrivateNetworks: false`.
...
- ~~SSRF / private-network protection for `web_fetch`~~ — landed as [`blockPrivateNetworks`](2026-08-25-web-fetch-ssrf-blocking.md) (pre-flight resolution, connect-time re-validation in the dispatcher, per-hop checks, embedded-IPv4 handling); the resolver-trust limits that remain are recorded there.
